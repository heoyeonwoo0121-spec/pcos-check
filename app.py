...
inputs = {}
with st.form("survey"):
    st.markdown("##### 1. 아래 증상이 있나요? (해당하면 체크)")
    ...
    with st.expander("▸ 추가 정보 (알고 있으면 입력 — 정확도가 조금 올라갑니다)"):
    ...
    submitted = st.form_submit_button("위험도 계산하기", type="primary", use_container_width=True)
...
if submitted:
    ...
    if proba>=0.5: band,color="높음","#C0392B"
    elif proba>=0.3: band,color="중간","#E67E22"
    else: band,color="낮음","#27AE60"
    st.markdown(
        ...
        f"<span style='font-size:1.2rem;color:{color};'>(위험 수준: {band})</span></div>",
